...
@section('title', "Detalhes do Administrador {$data->name}")

@section('content')
<h1 class="w-full text-3xl text-black pb-6">
    Detalhes do Administrador  <strong>{{ $data->name }}</strong>
</h1>
...
<div class="container">
    <div class="md:flex no-wrap md:-mx-2 ">
        ...
        <div class="w-full md:w-9/12 mx-2 h-64">
            ...
            <div class=" w-full">
                <form class="rounded float-right" action="{{ route('admin.destroy', $data->id) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <div class="mt-6">
                        <button class="px-4 py-1 text-white font-light tracking-wider bg-red-900 rounded" type="submit">
                            Deletar o Administrador {{ $data->name }}
                        </button>
                    </div>
                </form>
            </div>
